// Seeds the 114 surahs into the database. Run once after creating the schema.
const pool = require('./pool');

const surahs = [
  [1,'الفاتحة','Al-Fatihah',7],[2,'البقرة','Al-Baqarah',286],[3,'آل عمران','Aal-E-Imran',200],
  ...
  [40,'غافر','Ghafir',85],[41,'فصلت','Fussilat',54],[42,'الشورى','Ash-Shuraa',53],
  ...
  [73,'المزمل','Al-Muzzammil',20],[74,'المدثر','Al-Muddathir',56],[75,'القيامة','Al-Qiyamah',40],
...
async function seed() {
  ...
  try {
    ...
    for (const [number, ar, en, ayahs] of surahs) {
      await client.query(
        `INSERT INTO surahs (number, name_arabic, name_english, total_ayahs)
         VALUES ($1,$2,$3,$4) ON CONFLICT (number) DO NOTHING`,
        [number, ar, en, ayahs]
      );
    }
    await client.query('COMMIT');
    console.log('✅ Seeded 114 surahs successfully.');
  } catch (err) {
